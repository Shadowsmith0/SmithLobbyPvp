...
import us.asooner.lobbypvp.utils.AjParkourHook;
...
public class ItemManager {
    ...
    private ItemStack build(String path, String tag) {
        ...
        if (meta != null) {
            ...
            if (cfg.getBoolean("branding.item-lore.enable", true)) {
                String loreLine = cfg.getString("branding.item-lore.text", "");
                if (!loreLine.isEmpty()) {
    ...
    private void playEquipSound(Player player) {
        var cfg = plugin.getConfig();
        if (!cfg.getBoolean("equip-sound.enable", true)) return;
        String soundName = cfg.getString("equip-sound.sound", "ITEM_ARMOR_EQUIP_DIAMOND");
        try {
            player.playSound(player.getLocation(), org.bukkit.Sound.valueOf(soundName), 1f, 1f);
        } catch (IllegalArgumentException ex) {
            plugin.getLogger().warning("Invalid equip-sound in config: " + soundName);
        }
    }
    ...
    public void ensureKitIntact(Player player, boolean shouldHaveArmor) {
        ...
        if (shouldHaveArmor) {
            if (cfg.getBoolean("items.helmet.enable", true) && inv.getHelmet() == null) {
                inv.setHelmet(buildHelmet());
            }
            if (cfg.getBoolean("items.chestplate.enable", true) && inv.getChestplate() == null) {
                inv.setChestplate(buildChestplate());
            }
            if (cfg.getBoolean("items.leggings.enable", true) && inv.getLeggings() == null) {
                inv.setLeggings(buildLeggings());
            }
            if (cfg.getBoolean("items.boots.enable", true) && inv.getBoots() == null) {
                inv.setBoots(buildBoots());
            }
        }

        int weaponSlot = cfg.getInt("items.weapon.slot", 0);
